...
void moveForward(int dist) {
  long targetTicks = cmToEncoderTicks * dist;
  ...
  while (true) {
    ...
    if (avgTicks >= targetTicks) break;   // stop when distance is done

    // --- Exponential Speed Control ---
    float progress = avgTicks / (float)targetTicks;
    int dynamicSpeed = baseSpeed - pow(progress, slowRate) * (baseSpeed - minSpeed);
    dynamicSpeed = constrain(dynamicSpeed, minSpeed, baseSpeed);

    // --- Sensor update ---
    updateSensors();
    if (distFront < frontThresh) break;   // emergency stop
    ...
    if (hasLeftWall && hasRightWall) {
      wallError = (float)(distLeft - distRight);
    } else if (hasRightWall) {
      wallError = (float)(targetRightDist - distRight);
    } else if (hasLeftWall) {
      wallError = (float)(distLeft - targetLeftDist);
    } else {
      wallError = 0; // no walls
    }

    wallInt += wallError;
    wallInt = constrain(wallInt, -1000, 1000);
    float wallDeriv = wallError - wallPrev;
    wallPrev = wallError;
    wallPIDValue = wallKp * wallError + wallKi * wallInt + wallKd * wallDeriv;

    // --- Encoder PID ---
    encoderError = (float)(leftTicks - rightTicks);
    encoderInt += encoderError;
    encoderInt = constrain(encoderInt, -1000, 1000);
    float encDeriv = encoderError - encoderPrev;
    encoderPrev = encoderError;
    float encoderPIDValue = encKp * encoderError + encKi * encoderInt + encKd * encDeriv;

    // --- Merge PID corrections ---
    float encoderWeight = 0.6; // prioritise straight motion
    float wallWeight = 0.4; // prioritise wall correction
    float totalCorrection = encoderPIDValue;
    if (wallPIDValue) totalCorrection = (encoderWeight * encoderPIDValue) + (wallWeight * wallPIDValue);
    Serial.printf("%d %d %d %d %d \n", leftTicks, rightTicks, wallPIDValue, encoderPIDValue, totalCorrection);
